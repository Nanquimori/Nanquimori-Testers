...
function confirmTester(campaign, email) {
  ...
  MailApp.sendEmail({
    ...
    name: SETTINGS.senderName,
    htmlBody: `<div style="max-width:620px;font-family:Arial,sans-serif;line-height:1.65"><h2>Seu cadastro foi recebido.</h2><p>Você solicitou uma vaga no teste fechado do <b>${safe(campaign.name)}</b> — ${safe(campaign.role)}.</p><p>Aguarde a inclusão do seu e-mail. Depois use a mesma Conta Google no celular:</p><p><a href="${safe(campaign.testUrl)}"><b>1. Aceitar o convite em Português (Brasil)</b></a></p><p style="padding:14px;background:#fff3e8;border-left:4px solid #b6382d"><b>Atenção:</b> se aparecer “Leave the program” ou “Sair do programa”, você já aceitou. Não clique nessa opção, pois ela remove você do teste.</p><p><a href="${safe(campaign.storeUrl)}"><b>2. Instalar no Google Play</b></a></p><h3>Durante os ${campaign.testDays} dias</h3><ul><li>permaneça inscrito;</li><li>use o app em dias diferentes;</li><li>mantenha a versão atualizada;</li><li>também se cadastre no outro aplicativo da rodada;</li><li>envie feedback para <a href="mailto:${safe(feedback)}">${safe(feedback)}</a>.</li></ul><p>Informe o app, modelo do aparelho, Android, passos do problema e uma captura quando possível.</p><p>Obrigado por ajudar um projeto independente.</p></div>`,
  });
}
